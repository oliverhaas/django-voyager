from __future__ import annotations

import asyncio
import logging
from datetime import timedelta
from io import BytesIO

import httpx
import waffle
from asgiref.sync import sync_to_async
from django.core.files.base import ContentFile
from django.db.models import Avg, Count
from django.http import Http404
from django.utils import timezone
from PIL import Image, ImageFilter
from tenacity import retry, stop_after_attempt, wait_exponential

from lab.models import Accelerator, Collision, EventImage, Experiment, ProcessingStatus

logger = logging.getLogger(__name__)


async def ingest_collision(*, experiment_id: int, data: dict) -> Collision:
    """Validate and create a collision event for the given experiment."""
    try:
        experiment = await Experiment.objects.aget(pk=experiment_id)
    except Experiment.DoesNotExist as e:
        raise Http404 from e
    event = await Collision.objects.acreate(experiment=experiment, **data)
    await refresh_experiment_stats(experiment.pk)
    return event


async def refresh_experiment_stats(experiment_id: int) -> None:
    """Update denormalized stats on an experiment from its collision events."""
    stats = await Collision.objects.filter(experiment_id=experiment_id).aaggregate(
        total=Count("id"),
        avg_energy=Avg("energy_gev"),
    )
    await Experiment.objects.filter(pk=experiment_id).aupdate(
        total_events=stats["total"],
        avg_energy_gev=stats["avg_energy"],
    )


@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=1, max=10))
async def _send_webhook(url: str, payload: dict) -> None:
    """Send a webhook POST request with retry logic."""
    async with httpx.AsyncClient() as client:
        response = await client.post(url, json=payload, timeout=10)
        response.raise_for_status()


async def dispatch_webhook(*, accelerator_id: int, event_type: str, payload: dict) -> bool:
    """Send webhook notification if the feature flag is enabled."""
    if not await sync_to_async(waffle.switch_is_active)("enable_webhook_notifications"):
        logger.info("Webhook notifications disabled via feature flag")
        return False

    try:
        accelerator = await Accelerator.objects.aget(pk=accelerator_id)
    except Accelerator.DoesNotExist as e:
        raise Http404 from e
    if not accelerator.webhook_url:
        logger.info("No webhook URL configured for accelerator %s", accelerator.name)
        return False

    webhook_payload = {
        "event_type": event_type,
        "accelerator": accelerator.name,
        "data": payload,
    }
    await _send_webhook(accelerator.webhook_url, webhook_payload)
    logger.info("Webhook dispatched to %s for %s", accelerator.webhook_url, event_type)
    return True


async def process_event_image(event_image_id: int) -> None:
    """Generate thumbnail and processed version of an event image."""
    try:
        image = await EventImage.objects.aget(pk=event_image_id)
    except EventImage.DoesNotExist as e:
        raise Http404 from e
    image.processing_status = ProcessingStatus.PROCESSING
    await image.asave()

    try:
        img = await asyncio.to_thread(Image.open, image.original_image)

        def _process_image(img: Image.Image) -> tuple[bytes, bytes]:
            thumb = img.copy()
            thumb.thumbnail((300, 300))
            thumb_io = BytesIO()
            thumb.save(thumb_io, format="PNG")

            processed = img.filter(ImageFilter.FIND_EDGES)
            proc_io = BytesIO()
            processed.save(proc_io, format="PNG")
            return thumb_io.getvalue(), proc_io.getvalue()

        thumb_data, proc_data = await asyncio.to_thread(_process_image, img)

        await sync_to_async(image.thumbnail.save)(
            f"thumb_{image.pk}.png",
            ContentFile(thumb_data),
            save=False,
        )
        await sync_to_async(image.processed_image.save)(
            f"processed_{image.pk}.png",
            ContentFile(proc_data),
            save=False,
        )

        image.processing_status = ProcessingStatus.COMPLETED
        await image.asave()
    except Exception:
        logger.exception("Failed to process image %s", event_image_id)
        image.processing_status = ProcessingStatus.FAILED
        await image.asave()


async def cleanup_orphaned_images() -> int:
    """Delete failed images older than 7 days. Returns count of deleted images."""
    cutoff = timezone.now() - timedelta(days=7)
    images = EventImage.objects.filter(
        processing_status=ProcessingStatus.FAILED,
        created_at__lt=cutoff,
    )
    count, _ = await images.adelete()
    logger.info("Cleaned up %d orphaned images", count)
    return count
